...
- **複数モデル対応**: Gemini 3.6 Flash、3.1 Pro、Gemma 4、CLIモデル
...
```lua
{
  ...
  config = function()
    require("gemini_helper").setup({
      ...
      model = "gemini-3.6-flash", -- デフォルトモデル
...
require("gemini_helper").setup({
  ...
  model = "gemini-3.6-flash",  -- 使用するモデル
...
| モデル | 説明 |
|--------|------|
| `gemini-3.6-flash` | 高速なエージェント・マルチモーダルタスク向けの最新安定版（デフォルト、推奨） |
