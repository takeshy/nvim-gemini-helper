...
- **複数モデル対応**: Gemini 3.8 Flash、3.1 Pro、Gemma 4、CLIモデル
...
```lua
{
  ...
  config = function()
    require("gemini_helper").setup({
      ...
      model = "gemini-3.8-flash", -- デフォルトモデル
...
require("gemini_helper").setup({
  ...
  model = "gemini-3.8-flash",  -- 使用するモデル
...
| モデル | 説明 |
|--------|------|
| `gemini-3.8-flash` | 長期的なエンジニアリング・エージェントタスク向けの最も高性能なFlashモデル（デフォルト、推奨） |
| `gemini-3.1-pro-preview` | 最新のフラッグシップモデル、1Mコンテキスト、最高性能 |
| `gemini-3.1-pro-preview-customtools` | カスタムツールとbash向けに最適化されたエージェントワークフロー |
| `gemini-3.5-flash` | エージェント・コーディングタスク向けの安定した高性能モデル |
| `gemini-3.5-flash-lite` | 高スループット向けの最新低レイテンシ・低コストモデル |
...
| `gemini-3.5-flash-lite` | 無料枠の最新低レイテンシ・低コストモデル |
